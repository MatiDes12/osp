...
export function digestEmailTemplate(params: {
  ...
  const rows = params.events
    .map(
      (e) => `
    <tr>
      <td style="padding:8px 12px;border-bottom:1px solid #e4e4e7;font-size:13px;">${e.type}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #e4e4e7;font-size:13px;">${e.cameraName}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #e4e4e7;font-size:13px;">${e.severity}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #e4e4e7;font-size:13px;">${e.timestamp}</td>
    </tr>`,
    )
    .join("");

  const content = `
    <h2 style="margin:0 0 16px;font-size:18px;font-weight:600;">${params.period} Digest &mdash; ${params.tenantName}</h2>
    <p style="margin:0 0 16px;">Here's a summary of events from the past ${params.period.toLowerCase()}.</p>
    <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e4e4e7;border-radius:6px;overflow:hidden;">
      <thead>
        <tr style="background:#f4f4f5;">
          <th style="padding:8px 12px;text-align:left;font-size:12px;font-weight:600;color:${MUTED_COLOR};">Event</th>
          <th style="padding:8px 12px;text-align:left;font-size:12px;font-weight:600;color:${MUTED_COLOR};">Camera</th>
          <th style="padding:8px 12px;text-align:left;font-size:12px;font-weight:600;color:${MUTED_COLOR};">Severity</th>
          <th style="padding:8px 12px;text-align:left;font-size:12px;font-weight:600;color:${MUTED_COLOR};">Time</th>
        </tr>
      </thead>
      <tbody>${rows}</tbody>
    </table>
    <p style="margin:16px 0 0;color:${MUTED_COLOR};font-size:13px;">
      Total events: <strong>${params.events.length}</strong>. View all events in your dashboard.
    </p>
  `;
