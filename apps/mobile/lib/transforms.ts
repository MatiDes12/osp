...
export function transformCamera(raw: Raw): Camera {
  return {
    ...
    location: transformCameraLocation(
      (raw["location"] as Raw | undefined) ?? {},
    ),
    capabilities: transformCameraCapabilities(
      (raw["capabilities"] as Raw | undefined) ?? {},
    ),
...
function transformCameraConfig(raw: Raw): CameraConfig {
  return {
    recordingMode: str(
      pick(raw["recording_mode"], raw["recordingMode"], "motion"),
    ) as CameraConfig["recordingMode"],
    motionSensitivity: num(
      pick(raw["motion_sensitivity"], raw["motionSensitivity"], 5),
    ),
...
function transformEvent(raw: Raw): OSPEvent {
  return {
    ...
    snapshotUrl: pick(raw["snapshot_url"], raw["snapshotUrl"], null) as
      | string
      | null,
...
function transformUser(raw: Raw): User {
  return {
    ...
    cameraIds: pick(raw["camera_ids"], raw["cameraIds"], null) as
      | string[]
      | null,
    preferences: transformUserPreferences(
      (raw["preferences"] as Raw | undefined) ?? {},
    ),
    lastLoginAt: pick(raw["last_login_at"], raw["lastLoginAt"], null) as
      | string
      | null,
    createdAt: str(pick(raw["created_at"], raw["createdAt"], "")),
    updatedAt: str(pick(raw["updated_at"], raw["updatedAt"], "")),
...
function transformUserPreferences(raw: Raw): UserPreferences {
  return {
    theme: str(
      pick(raw["theme"], raw["theme"], "system"),
    ) as UserPreferences["theme"],
    notificationsEnabled: bool(
      pick(raw["notifications_enabled"], raw["notificationsEnabled"], true),
    ),
    defaultGridSize: num(
...
export function transformTenant(raw: Raw): Tenant {
  return {
    ...
    settings: transformTenantSettings(
      (raw["settings"] as Raw | undefined) ?? {},
    ),
    branding: transformTenantBranding(
      (raw["branding"] as Raw | undefined) ?? {},
    ),
    logoUrl: pick(raw["logo_url"], raw["logoUrl"], null) as string | null,
    customDomain: pick(raw["custom_domain"], raw["customDomain"], null) as
      | string
      | null,
...
function transformTenantSettings(raw: Raw): TenantSettings {
  const notifPrefs = pick(
    raw["notification_preferences"],
    raw["notificationPreferences"],
    {},
  ) as Raw;
  return {
    defaultRetentionDays: num(
      pick(raw["default_retention_days"], raw["defaultRetentionDays"], 7),
    ),
    defaultRecordingMode: str(
      pick(
        raw["default_recording_mode"],
        raw["defaultRecordingMode"],
        "motion",
      ),
    ) as TenantSettings["defaultRecordingMode"],
    defaultMotionSensitivity: num(
      pick(
        raw["default_motion_sensitivity"],
        raw["defaultMotionSensitivity"],
        5,
      ),
    ),
    ...
    notificationPreferences: {
      emailDigest: str(
        pick(notifPrefs["email_digest"], notifPrefs["emailDigest"], "none"),
      ) as TenantSettings["notificationPreferences"]["emailDigest"],
      pushEnabled: bool(
        pick(notifPrefs["push_enabled"], notifPrefs["pushEnabled"], true),
      ),
    },
  };
}
...
function transformTenantBranding(raw: Raw): TenantBranding {
  return {
    primaryColor: str(
      pick(raw["primary_color"], raw["primaryColor"], "#3b82f6"),
    ),
    accentColor: str(pick(raw["accent_color"], raw["accentColor"], "#2563eb")),
    fontFamily: pick(raw["font_family"], raw["fontFamily"], null) as
      | string
      | null,
    faviconUrl: pick(raw["favicon_url"], raw["faviconUrl"], null) as
      | string
      | null,
  };
}
